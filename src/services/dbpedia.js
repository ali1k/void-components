...
export default {
    ...
    read: (req, resource, params, config, callback) => {
        if (resource === 'dbpedia.lookup') {
            let lookupAddr = [{host: 'lookup.dbpedia.org'}];
            ...
            //send request
            rp({method: 'get', headers: {'Accept': 'application/json'}, accept: 'application/json', uri: 'http://' + lookupAddr[0].host + '/api/search.asmx/PrefixSearch?QueryClass=' + lookupClass + '&MaxHits=5&QueryString=' + query}).then(function(res){
                callback(null, {
